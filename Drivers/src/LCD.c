...
void LCD_Init(void){
	/*Set LCD_RSPin,LCD_RWPin,LCD_EPin and LCD_DataPort (Pins1-8) as output pins */
...
void LCD_SendCommand(unsigned char command){
	...
	for(int MiniDelay=0;MiniDelay<5000;MiniDelay++);
	for (int DataPortCounter=0;DataPortCounter<8;DataPortCounter++){
		GPIO_WritePin(LCD_DataPort,DataPortCounter,command & (1<<DataPortCounter));
	}
	for(int MiniDelay=0;MiniDelay<5000;MiniDelay++);
...
void LCD_DisplayCharacter(unsigned char data){
	...
	for(int MiniDelay=0;MiniDelay<5000;MiniDelay++);
	for (int DataPortCounter=0;DataPortCounter<8;DataPortCounter++){
		GPIO_WritePin(LCD_DataPort,DataPortCounter,data & (1<<DataPortCounter));
	}
	for(int MiniDelay=0;MiniDelay<5000;MiniDelay++);
	GPIO_WritePin(LCD_EPort,LCD_EPin,0);
	for(int MiniDelay=0;MiniDelay<5000;MiniDelay++);
